...
void	calc_x_coord_tex(t_data *data)
{
	...
	if (data->ray.hit_side == 0 && data->ray.ray_dirx > 0)
		data->draw.wall.tex_x = data->draw.wall.select_tex.tex_width - data->draw.wall.tex_x - 1;
	if (data->ray.hit_side == 1 && data->ray.ray_diry < 0)
		data->draw.wall.tex_x = data->draw.wall.select_tex.tex_width - data->draw.wall.tex_x - 1;
}
...
void	color_x_stripe(t_data *data, int x)
{
	...
	{
		data->draw.wall.tex_y = (int)tex_pos & (data->draw.wall.select_tex.tex_height - 1);
		tex_pos += step;
		color = (*(int *)(data->draw.wall.select_tex.addr + (4
						* data->draw.wall.select_tex.tex_width * (int)data->draw.wall.tex_y)
